...
if ($setPost['idCliente'] != null) {
    switch ($acao) {
        case 'create':
            $create = new Create;
            $read = new Read;
            $read->exeRead("carrinho", "WHERE idProduto = :idProduto AND idCliente = :idCliente", "idProduto={$setPost['idProduto']}&idCliente={$setPost['idCliente']}");
            if ($read->getRowCount() > 0) {
                $jSon['error'] = true;
                $jSon['mensagem'] = "Produto NAO inserido ao carrinho";
            } else {
                $create->exeCreate("carrinho", $setPost);
                $jSon['error'] = false;
